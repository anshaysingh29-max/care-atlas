...
export default function PartnerShell({ children, title, subtitle, action }) {
  ...
  const permitted = Boolean(
    user &&
    isPartnerRole(userProfile?.role) &&
    partnerProfile &&
  ...
  const isPatientPartner = userProfile?.role === USER_ROLES.PATIENT;
  ...
  return <section className="phase7a-partner-app">
    <div className="container phase7a-partner-shell">
      <aside className="phase7a-partner-sidebar">
        <div className="phase7a-partner-brand"><span><Handshake size={20}/></span><div><strong>CareAtlas Partners</strong><small>{isPatientPartner?'Patient + referral partner':'Referral network'}</small></div></div>
        ...
        {isPatientPartner && <Link href="/patient" className="phase7b-back-patient">← Back to patient portal</Link>}
      ...
      <main className="phase7a-partner-main">
        ...
        {!approved && <div className="phase7a-review-banner"><ShieldCheck size={18}/><div><strong>Your application is being reviewed.</strong><span>Your referral code is reserved, but attribution and lead submission only activate after CareAtlas approves the account.</span></div></div>}
        {children}
      </main>
    </div>
  </section>;
