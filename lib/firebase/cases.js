...
import { attachReferralToCase } from './referrals';
...
export async function createPatientCase({ form, treatmentName }) {
  ...
  const casePayload = {
    ...
    patientPreferredHospitalIds: Array.from(new Set(form.preferredHospitalIds || [])).filter(Boolean).slice(0, 5),
    careNavigatorContext: form.navigatorSpecialtyId ? {
      specialtyId: String(form.navigatorSpecialtyId).slice(0, 100),
      algorithmVersion: String(form.navigatorMatchVersion || '').slice(0, 120),
  ...
  // Affiliate attribution must never block the patient's medical case submission.
  try {
    await attachReferralToCase({
      caseId: caseRef.id,
      caseNumber,
      form,
      treatmentName: treatmentName || form.treatment,
    ...
    console.error('CareAtlas referral attribution could not be attached.', referralError);
  }
