...
import { hasHospitalSharingConsent } from './consents';
...
export async function getHospitalCaseConsentState(caseId, hospitalId) {
...
export async function getHospitalCaseDocuments(caseId, hospitalId) {
  await getHospitalCase(caseId, hospitalId);
  const consentState = await getHospitalCaseConsentState(caseId, hospitalId);
  if (!hasHospitalSharingConsent(consentState)) return [];
...
export async function createHospitalTreatmentPlan({ hospitalId, caseId, form }) {
  ...
  batch.set(notificationRef, {
    ...
    hospitalId: profile.hospitalId,
    type: 'treatment_plan',
    title: 'New treatment plan received',
    body: `${hospital?.name || profile.hospitalName || 'An assigned hospital'} submitted a treatment plan for ${caseRecord.caseNumber || 'your case'}.`,
    createdBy: user.uid,
...
export async function createHospitalConsultation({ hospitalId, caseId, form }) {
  ...
  batch.set(notificationRef, {
    ...
    body: `${payload.doctorName || 'An assigned doctor'} proposed a ${payload.mode.replace('_', ' ')} consultation.`,
    createdBy: user.uid,
    createdByRole: profile.role,
    source: 'hospital_web',
    readAt: null,
...
export async function updateHospitalConsultationStatus({ hospitalId, consultationId, status }) {
  ...
  await addDoc(collection(db, 'notifications'), {
    ...
    body: `Your consultation is now ${status.replace('_', ' ')}.`,
    createdBy: user.uid,
    createdByRole: profile.role,
    source: 'hospital_web',
    readAt: null,
