...
export function isAdminRole(role) {
  return role === 'careatlas_admin' || role === 'super_admin';
}
...
export async function getAdminCaseConsentState(caseId) {
  requireStaff();
...
export async function getAdminCaseDocuments(caseId) {
  ...
  const consentSnapshot = await getDoc(doc(db, 'caseConsentStates', caseId));
  if (!consentSnapshot.exists() || consentSnapshot.data().medicalDataProcessing !== true) return [];
...
export async function getAdminAuditLogs() {
  requireStaff();
  const snapshot = await getDocs(collection(getFirebaseDb(), 'auditLogs'));
  return snapshot.docs
    .map(item => ({ id: item.id, ...item.data() }))
    .sort((a, b) => timestampMillis(b.createdAt) - timestampMillis(a.createdAt));
}
...
export async function updateAdminCaseOperations({
  ...
  const db = getFirebaseDb();
  const current = await getAdminCase(caseId);
  ...
  batch.update(doc(db, 'cases', caseId), payload);
  batch.set(auditRef, {
    actorId: user.uid,
  ...
  batch.set(notificationRef, {
    ...
    caseId,
    type: 'case_update',
    title: 'Your CareAtlas case was updated',
    body: `${stageLabel(currentStage)} · ${statusLabel(status)}`,
    createdBy: user.uid,
  ...
  // Keep the privacy-safe partner referral timeline aligned with case operations.
  try {
    const referralRef = doc(db, 'referrals', caseId);
    const referralSnapshot = await getDoc(referralRef);
    if (referralSnapshot.exists()) {
      await updateDoc(referralRef, { currentStage, caseStatus: status, updatedAt: serverTimestamp() });
    }
  } catch (referralError) {
    console.error('Could not synchronize referral journey state.', referralError);
  }
