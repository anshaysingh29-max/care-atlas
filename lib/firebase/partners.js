...
const PARTNER_CAPABLE_ROLES = Object.freeze([
  USER_ROLES.PARTNER,
  USER_ROLES.PATIENT
]);
...
export function isPartnerRole(role) {
  return PARTNER_CAPABLE_ROLES.includes(role);
}

export function getPartnerLevel({ referrals = [], commissions = [] } = {}) {
  const verified = referrals.filter(item => item.referralStatus === 'treatment_verified').length;
  const paid = commissions.filter(item => item.status === 'paid').length;
  const score = Math.max(verified, paid);
  if (score >= 10) return { key: 'pro', label: 'Pro Partner', nextAt: null, score };
  if (score >= 3) return { key: 'growth', label: 'Growth Partner', nextAt: 10, score };
  return { key: 'starter', label: 'Starter Partner', nextAt: 3, score };
}

function partnerApplicationPayload({ user, displayName, country, phone, organization, partnerType, source }) {
  return {
    partnerId: user.uid,
    email: user.email || '',
...
export async function registerPartner({ name, email, password, country, phone, organization, partnerType, acceptedTerms }) {
  ...
  try {
    ...
    const partner = partnerApplicationPayload({
      user,
      displayName,
      country,
      phone,
...
export async function registerCurrentPatientAsPartner({
  phone,
  organization,
  partnerType,
  acceptedTerms
  ...
  if (!acceptedTerms) {
    const error = new Error('Accept the CareAtlas Partner terms and referral disclosure before applying.');
    error.code = 'careatlas/partner-terms-required';
    throw error;
  }
  ...
  const patientProfile = patientSnapshot.exists() ? patientSnapshot.data() : null;

  if (userProfile?.role !== USER_ROLES.PATIENT || !patientProfile) {
    throw new Error('Only an existing CareAtlas patient account can use this application path.');
  }
  if (partnerSnapshot.exists()) {
    return { id: partnerSnapshot.id, ...partnerSnapshot.data() };
  }

  const partner = partnerApplicationPayload({
    user,
    displayName: patientProfile.displayName || user.displayName || '',
    country: patientProfile.country || '',
    phone: phone || patientProfile.phone || '',
    organization,
    partnerType: partnerType || 'Former patient / community advocate',
...
export async function signInPartner({ email, password }) {
  ...
  if (
    !userProfile ||
    !PARTNER_CAPABLE_ROLES.includes(userProfile.role) ||
    !partnerProfile ||
    ['rejected', 'suspended'].includes(partnerProfile.status)
  ) {
    await signOut(getFirebaseAuth());
    const error = new Error('This account does not have CareAtlas Partner access.');
...
export async function getPartnerLeadRows() {
  const user = getFirebaseAuth().currentUser;
  if (!user) throw new Error('Partner sign-in is required.');
  const snapshot = await getDocs(query(collection(getFirebaseDb(), 'partnerLeads'), where('partnerId', '==', user.uid)));
  return snapshot.docs.map(item => ({ id: item.id, ...item.data() }))
    .sort((a, b) => timestampMillis(b.updatedAt || b.createdAt) - timestampMillis(a.updatedAt || a.createdAt));
}

export async function getPartnerDashboardData() {
  const [profile, referrals, commissions, leads] = await Promise.all([
    getPartnerProfile(),
    getPartnerReferrals(),
  ...
  const treatmentVerified = referrals.filter(item => item.referralStatus === 'treatment_verified').length;
  const level = getPartnerLevel({ referrals, commissions });
  return { profile, referrals, commissions, leads, approved, paid, pending, qualified, treatmentVerified, level };
}
...
export function buildPartnerReferralUrl(code, campaign = '') {
  if (typeof window === 'undefined' || !code) return '';
  const path = window.location.pathname;
  const partnerIndex = path.indexOf('/partner');
  const patientIndex = path.indexOf('/patient');
  const cutAt = [partnerIndex, patientIndex].filter(value => value >= 0).sort((a, b) => a - b)[0];
  const basePath = Number.isInteger(cutAt) ? path.slice(0, cutAt) : '';
  const params = new URLSearchParams({ ref: code });
  const normalizedCampaign = sanitizeCampaign(campaign);
