...
export default function DestinationsPage() {
  return (
    <>
      ...
      <section className="section"><div className="container"><SectionTitle title="Initial CareAtlas destination network" text="Patients can originate from anywhere; provider coverage begins with these selected destinations."/><div className="cards-grid destination-grid">{destinations.map(d => <DestinationCard key={d.slug} destination={d}/>)}</div></div></section>
    </>
  );
}
